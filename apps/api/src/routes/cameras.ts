...
export const cameraRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post('/:id/start-stream', { preHandler: [server.authenticate] }, async (request, reply) => {
    ...
    if (result.error) {
      ...
      const isHealthError = Object.prototype.hasOwnProperty.call({ RTSP_SUB_NOT_FOUND: 1, CODEC_UNSUPPORTED_HEVC: 1, AUTH_FAILED: 1, OFFLINE: 1, RTSP_MAIN_NOT_FOUND: 1, TRANSCODING_DISABLED: 1, TRANSCODE_LIMIT_REACHED: 1, CAMERA_OFFLINE: 1, TRANSCODE_NOT_READY: 1 }, result.error.code)
