...
interface TranscodeInFlight {
  ...
  /**
   * Intento que escribió este estado. Sin identidad, un arranque viejo que se
   * reanuda tarde pisaba el single-flight de otro que ya había tomado el path.
   * El supervisor escribe sin `attemptId`: su estado describe al proceso, no a
   * una petición, y cualquier intento puede reemplazarlo.
   */
  attemptId?: string
}
...
interface StreamSession {
  ...
  generation: number
  /**
   * Secuencia de la ÚLTIMA petición autenticada que afirmó la propiedad de esta
   * sesión: la que la creó, la reutilizó, la adoptó como waiter o la latió.
   *
   * No se reutiliza `generation`, que expresa creación/completado y no el orden
   * de llegada HTTP. Acá hace falta lo segundo: un cierre sólo puede borrar una
   * sesión cuya propiedad NO sea posterior a él.
   *
   *   stop 1 entra y queda esperando autenticación
   *   start 2 entra y completa       → lastOwnerRequestSeq = 2
   *   stop 1 continúa                → 2 > 1 ⇒ NO la borra
   *
   * Sólo la actualiza actividad real de cliente, y siempre con Math.max: ni el
...
/**
 * ¿Puede este cierre borrar esta sesión?
 *
 * Sólo si la propiedad de la sesión NO es posterior al cierre. Si lo es, la
 * sesión fue creada o reafirmada por una petición que llegó después y debe
 * preservarse.
 */
function closeMayDelete(session: StreamSession, closeTicket: RequestTicket): boolean {
  return session.lastOwnerRequestSeq <= closeTicket.seq
}

/**
 * Relee la sesión vigente y decide si este cierre puede borrarla.
 *
 * Se relee del mapa a propósito: entre el snapshot y el borrado hubo
 * operaciones asíncronas, y la fila pudo ser reemplazada por otra petición
 * (misma clave, generación distinta). Decidir con el objeto capturado borraría
 * la sesión NUEVA.
 */
function resolveDeletable(
  key: string,
  expectedGeneration: number,
  closeTicket: RequestTicket,
): { deletable: false; reason: string } | { deletable: true; session: StreamSession } {
  const current = sessions.get(key)
  if (!current) return { deletable: false, reason: 'already_gone' }
  if (current.generation !== expectedGeneration) {
    return { deletable: false, reason: 'replaced_by_newer_generation' }
  }
  if (!closeMayDelete(current, closeTicket)) {
    return { deletable: false, reason: 'reaffirmed_by_newer_request' }
  }
  return { deletable: true, session: current }
}
...
// segunda pisaba a la primera. Sin esto, la cancelada podía llamar a
// removeStream después de que la otra registrara su sesión, dejando a un
 ...
 *
 * Devuelve `true` cuando el path se CONSERVÓ porque otro dueño válido lo usa
 * (sesión registrada, waiter o arranque en vuelo ajeno), y `false` cuando
...
function pathHasOwner(streamPath: string, excludeAttemptId?: string): boolean {
  // Un waiter tampoco adopta un arranque ya declarado FALLIDO: va a recibir el
  // fallo y retirarse, así que contarlo como dueño dejaría el path conservado
  // sin nadie detrás. Esto hace que la limpieza de un arranque fallido no
  // dependa de cuándo se reanuden los que esperaban.
  const waiterAdoptable =
...
/**
 * Escribe el estado del single-flight SÓLO si este intento sigue siendo su
 * dueño. Un arranque viejo que se reanuda tarde no puede pisar el estado del
 * que ya tomó el path. Devuelve si la escritura ocurrió.
 */
function setTranscodeInFlightOwned(
  streamPath: string,
  attemptId: string,
  next: Omit<TranscodeInFlight, 'attemptId'>,
): boolean {
  const current = transcodeInFlight.get(streamPath)
  if (current?.attemptId && current.attemptId !== attemptId) {
    console.info(
      `[transcode] inflight_write_skipped path=${streamPath}` +
      ` owner=${current.attemptId} writer=${attemptId} state=${next.state}`
    )
    return false
  }
  transcodeInFlight.set(streamPath, { ...next, attemptId })
  return true
}

/**
 * ¿El single-flight guardado ya NO describe la realidad del path?
 *
 * Conservar un path porque tiene dueño no autoriza a conservar cualquier
 * estado viejo: un `ready` sin proceso anunciaría un FFmpeg muerto, un
 * `starting` sin arranque válido bloquearía el cupo para siempre, y un
 * `failed` no describe a un path que se está conservando.
 */
function inFlightStateIsStale(streamPath: string, excludeAttemptId?: string): boolean {
  const f = transcodeInFlight.get(streamPath)
  if (!f) return false
  // READY sólo vale mientras haya proceso detrás.
  if (f.state === 'ready') return !isTranscodeProcessAlive(streamPath)
  if (f.state === 'starting') {
    // STARTING vale si OTRO arranque válido lo sostiene —el que se está
    // descartando no cuenta— o si un waiter puede adoptar su proceso vivo.
    // Sin la exclusión, el `starting` del intento descartado sobrevivía sin
    // nadie que lo resolviera y ocupaba cupo para siempre.
    const otroArranque  = hasValidInFlightOwner(streamPath, excludeAttemptId)
    const waiterAdopta  = hasWaiters(streamPath) && isTranscodeProcessAlive(streamPath)
    return !otroArranque && !waiterAdopta
  }
  // FAILED nunca describe a un path que se está conservando.
  return true
}
...
async function releaseUnownedStream(
  ...
  // La propiedad se comprueba ANTES de tocar nada. Antes se borraba
  // `transcodeInFlight` en la primera línea, así que un path CONSERVADO por
  // otra sesión, un waiter o un arranque válido perdía igual su readiness: el
  // proceso seguía vivo pero sin su estado compartido, y el arranque siguiente
  // no lo adoptaba o levantaba un segundo FFmpeg (A1.7).
  if (pathHasOwner(streamPath, excludeAttemptId)) {
    // Conservar no es conservar a ciegas. Un estado que ya no describe al path
    // vigente se descarta igual: es lo único que se toca de un path con dueño.
    if (inFlightStateIsStale(streamPath, excludeAttemptId)) {
      const dropped = transcodeInFlight.get(streamPath)?.state
      transcodeInFlight.delete(streamPath)
      console.info(
        `[live] inflight_state_dropped path=${streamPath} state=${dropped} reason=stale_while_kept`
      )
    }
  ...
  // Sin dueño: recién acá se retira el estado del path. El borrado ya no
  // necesita respetar identidades —no hay nada de nadie que proteger—, pero la
  // ESCRITURA sí (`setTranscodeInFlightOwned`).
  transcodeInFlight.delete(streamPath)
  ...
  //
  // Se RELEE después del `await` anterior: la lista capturada al principio de
  // la función ya envejeció, y con ella se podía retirar los paths de una
  // cámara que volvió a tener espectadores durante el borrado.
  if (toSessionTruths().some(s => s.cameraId === cameraId)) {
...
function isStartCancelled(streamPath: string): boolean {
  if (!inFlightStarts.has(streamPath)) return false
  return !hasValidInFlightOwner(streamPath)
}
 ...
 */
/**
 * Avanza una marca de cierre. NUNCA retrocede.
 *
 * Un `set` incondicional dejaba que un cierre viejo que terminaba tarde pisara
 * una marca más nueva:
 *
 *   close 1 entra y queda esperando
 *   start 2 entra
 *   close 3 entra y termina  → watermark = 3
 *   close 1 termina después  → watermark BAJA a 1
 *   → start 2 deja de parecer obsoleto → sesión fantasma
 *
 * Es un watermark: sólo sube. Una secuencia menor no toca nada, y una igual
 * tampoco reemplaza la información existente (revisión de #149 y #150).
 */
function advanceCloseWatermark(
  map: Map<string, CloseMark>,
  key: string,
  ticket: RequestTicket,
): void {
  const current = map.get(key)
  if (current && ticket.seq <= current.seq) return
  map.set(key, { atMs: Date.now(), seq: ticket.seq })
}

/**
 * Poda de marcas viejas. Se hace por EDAD del propio registro y sólo sobre lo
 * que realmente caducó: una petición antigua que se procesa tarde no puede
 * arrastrarse por delante una marca más nueva.
 */
function pruneCloseMarks(nowMs: number): void {
...
export function markViewClosed(
  ...
  advanceCloseWatermark(viewClosedAt, vKey(userId, viewId), ticket)
  pruneCloseMarks(Date.now())
}
...
function markTargetClosed(key: string, ticket: RequestTicket): void {
  advanceCloseWatermark(targetClosedAt, key, ticket)
}
...
/** Sólo tests: ¿queda historial de reinicios del supervisor para este path? */
export function __hasTranscodeRestartsForTest(streamPath: string): boolean {
  return transcodeRestarts.has(streamPath)
}

/** Sólo tests: ¿queda actividad de medio registrada para este path? */
export function __hasMediaActivityForTest(streamPath: string): boolean {
  return lastMediaActivity.has(streamPath)
}
...
/** Sólo tests: estado e identidad del single-flight de un path. */
export function __getTranscodeInFlightForTest(
  streamPath: string,
...
export function __setTranscodeInFlightForTest(
  streamPath: string,
  state: 'starting' | 'ready' | 'failed',
  attemptId?: string,
): (v: boolean) => void {
  ...
  transcodeInFlight.set(streamPath, { state, promise, resolve, attemptId })
  return resolve
}

/** Sólo tests: registra un arranque en vuelo sobre un path. */
export function __addInFlightStartForTest(
  streamPath: string, attemptId: string,
  o: { userId: string; viewId: string; cameraId: string; ticket: RequestTicket },
): void {
  addInFlightStart(streamPath, attemptId, {
    userId: o.userId, viewId: o.viewId, cameraId: o.cameraId,
    streamType: 'main_h264', ticket: o.ticket,
    sessionKey: sessionKey({ userId: o.userId, viewId: o.viewId, cameraId: o.cameraId, streamType: 'main_h264' }),
  })
}

/** Sólo tests: suma una pestaña esperando el arranque compartido del path. */
export function __addWaiterForTest(streamPath: string): void {
  addWaiter(streamPath)
}

/** Sólo tests: marca el path como publicado en MediaMTX. */
export function __markPathPublishedForTest(streamPath: string): void {
  publishedPaths.add(streamPath)
}

/**
 * Sólo tests: ejecuta la liberación tal cual la invoca el runtime.
 *
 * Es la unidad que A1.7 corrige, y sus entradas —sesiones, waiters, arranques
 * en vuelo y estado por path— son todas internas del módulo. Llamarla directo
 * permite fijar cada combinación sin montar una carrera distinta para cada una.
 */
export function __releaseUnownedStreamForTest(
  server: FastifyInstance,
  cameraId: string,
  streamPath: string,
  reason: string,
  ...
  return releaseUnownedStream(server, cameraId, streamPath, reason, excludeAttemptId)
}
...
async function startStreamCore(
  ...
  if (effectiveType === 'main_h264') {
    ...
    /**
     * Aborto del registro por cierre de la pestaña. ÚNICO punto: las cuatro
     * rutas de registro (proceso vivo reutilizado, listo, y las dos de
     * "parcialmente listo") deben comportarse igual. Antes eran copias
     * separadas y dos de ellas resolvían el single-flight en `false`, con lo
     ...
     */
    /**
     * Finalización ÚNICA de un arranque transcodificado exitoso.
     *
     * Recibe todo lo necesario para distinguir cuatro cosas que antes se
     * confundían: el path LOCAL que creó este intento, la sesión retenida en el
     * momento del registro, la sesión VIGENTE tras cualquier `await`, y el
     * `startAttemptId` de este intento.
     *
     * El defecto que corrige (revisión de #152): cuando un arranque más nuevo
     * reemplazaba la fila DESPUÉS del registro pero antes de que se reanudara
     * este `await`, la finalización detectaba el cambio, devolvía el path nuevo
     * y NUNCA liberaba el local. En esa secuencia `localPathDiscarded` fue
     * false durante el registro, así que no hubo limpieza previa: quedaban
     * huérfanos el FFmpeg, su `transcodeInFlight`, la info de origen, los
     * reinicios, la actividad y la publicación, consumiendo cupo sin dueño.
     */
    const finalizeTranscodeSuccess = async (opts: {
      /** Sesión retenida en el instante del registro. */
      registered: StreamSession
      /** Path que creó ESTE intento. Puede no ser el retenido. */
      localPath: string
      /** Identidad de este intento, para excluirse al comprobar propiedad. */
      attemptId: string
      /** Promesa del single-flight del path LOCAL, si la hubo. */
      inFlightPromise?: Promise<boolean>
      /** Resolución del single-flight del path LOCAL, si la hubo. */
      resolveLocalReady?: (v: boolean) => void
    }) => {
      // 1ª relectura: la fila pudo cambiar mientras se cedía el turno.
      let current = sessions.get(transcodeKey)
      if (!current) {
        console.info(
          `[transcode] finalize_aborted cameraId=${cameraId}` +
          ` reason=session_gone_after_register key=${transcodeKey}`
        )
        // El single-flight describe la disponibilidad de SU path: nadie lo tiene.
        opts.resolveLocalReady?.(false)
        return abortResult('finalize_session_gone')
      }

      if (current.streamPath === opts.localPath) {
        // Caso normal: el path local ES el retenido. Se marca listo y se
        // resuelve su propio single-flight como disponible. Sin `await` entre
        // esta relectura y el retorno.
        lastMediaActivity.set(current.streamPath, Date.now())
        // Con identidad: si otro intento ya tomó el path, no se le pisa el
        // estado (A1.7).
        setTranscodeInFlightOwned(current.streamPath, opts.attemptId, {
          state: 'ready',
          promise: opts.inFlightPromise ?? Promise.resolve(true),
          resolve: () => {},
        })
        opts.resolveLocalReady?.(true)
        const path = current.streamPath
        return { hlsUrl: getHlsUrl(path), webrtcUrl: getWebRtcUrl(path), streamPath: path, transcoded: true }
      }

      // REDIRIGIDA: otro arranque retuvo la clave con otro path. Hay que
      // liberar el local ANTES de responder, o queda huérfano ocupando cupo.
      console.info(
        `[transcode] finalize_repointed cameraId=${cameraId}` +
        ` from=${opts.localPath} to=${current.streamPath}` +
        ` registered=${opts.registered.streamPath} attempt=${opts.attemptId}`
      )
      // Se ESPERA la liberación (nada de `void`) y se excluye ÚNICAMENTE este
      // intento: si otra sesión, otro intento válido o un waiter poseen el path
      // local, `releaseUnownedStream` lo conserva. Es idempotente, así que
      // llamarla cuando el registro ya había limpiado no hace daño.
      ...
      if (!current) {
        console.info(
          `[transcode] finalize_aborted cameraId=${cameraId}` +
          ` reason=session_gone_after_release key=${transcodeKey}`
        )
        return abortResult('finalize_session_gone')
      }
      // Desde acá hasta el retorno no hay ningún `await`: la fila leída es la
      // que se responde. No se toca el estado del path retenido — es de otro
      // propietario, que ya lo marcó listo por su cuenta.
      const path = current.streamPath
      return { hlsUrl: getHlsUrl(path), webrtcUrl: getWebRtcUrl(path), streamPath: path, transcoded: true }
    }
    ...
    if (existingSession) {
      ...
      // pestaña se apropiaba de la sesión de la primera.
    ...
    if (inFlight?.state === 'starting') {
      ...
      if (ready) {
        ...
        return await finalizeTranscodeSuccess({
          registered: reg.session, localPath: streamPath, attemptId: startAttemptId,
        })
      }
      return { hlsUrl: '', webrtcUrl: '', streamPath: '',
    ...
    /**
     * Terminación ÚNICA de un arranque que ya había publicado (o intentado
     * publicar) el path.
     *
     * Antes cada salida fallida posterior a `publishTranscodedStream` hacía su
     * propia media limpieza —marcar `failed`, resolver el single-flight y, a
     * veces, matar FFmpeg— y ninguna llamaba a la liberación. Quedaban el path
     * pasivo en MediaMTX, `publishedPaths`, `transcodeSourceInfo`,
     * `transcodeRestarts` y `lastMediaActivity` sin dueño (A1.7).
     *
     * Se ejecuta UNA SOLA VEZ aunque coincidan una salida explícita, el `catch`
     * y el `finally`. No recibe `server`/`cameraId`/`streamPath`/`attemptId`
     * como parámetros a propósito: son los del cierre, así que no pueden
     * desincronizarse entre los cinco puntos de llamada — la duplicación de
     * esos valores es justamente lo que produjo los defectos de #153.
     */
    let startFailureHandled = false
    const cleanupFailedStart = async (reason: string): Promise<void> => {
      if (startFailureHandled) return
      startFailureHandled = true

      // 1. El single-flight de ESTE intento pasa a `failed`. Si otro intento ya
      //    se adueñó del path, la escritura se salta y su estado queda intacto.
      setTranscodeInFlightOwned(streamPath, startAttemptId, {
        state: 'failed', promise: readyPromise, resolve: resolveInFlight,
      })
      // 2. Los que esperaban se enteran ANTES de la limpieza. Un `failed` los
      //    saca de la lista de adoptantes en `pathHasOwner`, así que la
      //    liberación puede retirar el path sin depender de cuándo se reanuden.
    ...
    try {
      ...
      if (!ready) {
        ...
        if (manifestVisible) {
          ...
          return await finalizeTranscodeSuccess({
            registered: reg.session, localPath: streamPath, attemptId: startAttemptId,
            inFlightPromise: readyPromise, resolveLocalReady: resolveInFlight,
          })
        }
      ...
      // Misma limpieza que cualquier otra salida fallida, y el error ORIGINAL
      // se relanza intacto. Si una salida explícita ya la ejecutó, el guardia
      // la vuelve un no-op.
      await cleanupFailedStart('exception_after_publish')
      throw err
    } finally {
...
export async function stopStream(
  ...
  if (session && key) {
    ...
    // RELEER antes de borrar. Entre la captura de `session` y este punto hubo
    // awaits, y la fila pudo ser reemplazada o reafirmada por una petición
    // POSTERIOR a este cierre:
    //
    //   stop 1 entra y queda esperando autenticación
    //   start 2 entra y completa      → lastOwnerRequestSeq = 2
    ...
    console.info(
      ...
      ` viewId=${session.viewId} generation=${generation} closeSeq=${ticket.seq}` +
...
export async function reconcileView(
  ...
  for (const cameraId of visibleCameraIds) {
    ...
    if (existing) {
      ...
      reaffirmOwnership(existing, ticket)
...
export async function cleanupUserSessions(
  ...
  // FLUJO: el conjunto a terminar NO puede decidirse con un snapshot previo.
  // Entre el snapshot y el borrado hay awaits, y una sesión pudo ser
  // reemplazada o reafirmada por una petición POSTERIOR a este cierre. Por eso
  // primero se borra lo realmente elegible, y recién con el mapa YA
  // actualizado se decide qué procesos terminar (revisión de #149).
  const deleted: SessionTruth[] = []

  for (const session of targetSessions) {
    const key = sessionKey({ userId, viewId: session.viewId, cameraId: session.cameraId, streamType: session.streamType })
    // Releer el estado ACTUAL de esta clave y comprobar propiedad.
    const verdict = resolveDeletable(key, session.generation, ticket)
    if (!verdict.deletable) {
      console.info(
        `[live] cleanup_skipped cameraId=${session.cameraId} streamType=${session.streamType}` +
        ` viewId=${session.viewId} closeSeq=${ticket.seq} reason=${verdict.reason}`
      )
      continue
    }
    ...
    removed++
    console.info(
      `[stream-manager] view_session_closed cameraId=${current.cameraId}` +
      ` streamType=${current.streamType} viewId=${current.viewId}` +
      ` generation=${current.generation} closeSeq=${ticket.seq}` +
      ` reason=${viewId ? 'cleanup_view' : 'cleanup_stale'}`
    )
  }
  ...
  if (viewId) {
    const vk = vKey(userId, viewId)
    // Sólo si el view quedó REALMENTE sin sesiones. Si una petición posterior
    // creó o reafirmó una sesión en este mismo view, borrar sus índices la
    // dejaría sin `viewHeartbeat` y la siguiente limpieza la mataría por
    // `view_heartbeat_missing` (revisión de #149).
    const stillHasSessions = Array.from(sessions.values())
      .some(sess => sess.userId === userId && sess.viewId === viewId)
    if (stillHasSessions) {
      console.info(`[live] view_indexes_kept viewId=${viewId} reason=newer_sessions_present`)
    } else {
      viewCameras.delete(vk)
      viewHeartbeat.delete(vk)
    }
