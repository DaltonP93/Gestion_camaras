...
- `docs/audits/ROBUSTNESS_CYCLE2.md` es el inventario **previo** a las correcciones del Ciclo 2; contrastarlo con el código y con la sección del Ciclo 2 en `docs/PROJECT_DOCUMENTATION.md` antes de tratar un hallazgo como vigente.

## Línea base verificada

- Rama principal: `main`.
- HEAD observado: `1d5454f8ea93e6dcb4f693945ec268eedf01bd25`.
- PR [#162](https://github.com/DaltonP93/Gestion_camaras/pull/162), fusionado el 2026-09-04: importó el ciclo C22 (188 archivos, 30 commits), incluidos grants/revocación, Frigate opt-in, ONVIF/Hik-Connect, base de IA/analytics, núcleo nativo y trabajo preparatorio de A1.
- PR [#164](https://github.com/DaltonP93/Gestion_camaras/pull/164), fusionado el 2026-09-04: Ciclo 2 de robustez (25 archivos, 14 commits, 850 adiciones y 179 eliminaciones).
- El Ciclo 2 incorporó: enmascarado de IPs en logs, aislamiento por cámara al re-registrar streams, limpieza de sesión de medios al cerrar sesión, comparación timing-safe de `/metrics`, manejo de errores Redis y límite para snapshots de Frigate.
- También fijó versiones de imágenes, agregó healthchecks, recarga periódica de Nginx para certificados y evitó que `upgrade-to-https.sh` reemplace la configuración endurecida.
- Cambió dos defaults operativos aprobados: CORS sin allowlist queda limitado a localhost/same-origin, y contraseña PostgreSQL/credenciales iniciales de administrador dejan de tener defaults conocidos.
- Estos merges no prueban ni autorizan despliegue, migración, reinicio o activación de flags.

## Validación y checks observados

- GitHub Actions, ejecución [CI #196](https://github.com/DaltonP93/Gestion_camaras/actions/runs/33917487689) sobre el head del PR #164 `1db8a8c`: **success**.
...
- La descripción del PR registra además `tsc --noEmit` sin errores, 1.233 pruebas Vitest y validaciones YAML/bash; tratarlos como evidencia declarada del desarrollo, no como validación de hardware o producción.
- GitHub no registró reviews, comentarios de conversación ni comentarios inline en el PR #164.
- No se observaron contexts de status separados ni una ejecución asociada todavía al SHA de merge `1d5454f`.
...
## Precauciones operativas nuevas

- Definir explícitamente `CORS_ORIGINS` para cualquier frontend autorizado que no sea same-origin/localhost; si falta, el nuevo default puede bloquear orígenes externos que antes eran reflejados.
- Definir `POSTGRES_PASSWORD` antes de levantar/desplegar el stack. No recuperar el antiguo valor hardcodeado ni registrarlo en documentación.
- El seed usa `SEED_ADMIN_PASSWORD` si se entrega; si no, genera una contraseña aleatoria y la muestra una sola vez. Capturarla de forma segura y cambiarla tras el primer acceso.
- Producción también requiere una `NVR_CREDENTIAL_KEY` válida; verificar compatibilidad/migración de credenciales legacy y rollback sin registrar valores.
- Las imágenes ahora están fijadas a versiones concretas. Las actualizaciones futuras deben ser deliberadas y probadas, no depender de tags flotantes.

## Riesgos y pendientes confirmados

- MediaMTX sigue sin healthcheck interno; cambiarlo requiere decidir la estrategia/imagen.
- Los usuarios demo del seed conservan contraseñas conocidas y no deben considerarse cuentas aptas para producción.
- Siguen pendientes: CSP sin `unsafe-inline`, rate-limit compartido en Redis y prueba automatizada de acceso cruzado/IDOR.
...
2. Leer la documentación y el código del subsistema afectado; diferenciar informes pre-corrección de la línea base actual.
...
- Para cambios de CORS/deploy, validar explícitamente allowlist, variables requeridas, healthchecks y rollback en un entorno no productivo.
